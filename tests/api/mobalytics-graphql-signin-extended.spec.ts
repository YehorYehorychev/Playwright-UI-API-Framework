 ...
 */
async function sendSignIn(request: APIRequestContext, variables: Record<string, unknown>) {
 ...
 */
function isRejected(body: { data?: { signIn?: unknown } | null; errors?: unknown[] } | null) {
  if (!body) return true;
  return !!body.errors || body.data?.signIn === false || body.data?.signIn === null;
}
...
test.describe("GraphQL SignIn — Extended Scenarios", { tag: [Tags.api, Tags.auth] }, () => {
  ...
  test(
    ...
    { tag: Tags.regression },
    async ({ request }) => {
      const { body, response } = await test.step("Send SignIn with non-existent email", async () =>
        sendSignIn(request, {
  ...
  test(
    ...
    { tag: Tags.regression },
    async ({ request }) => {
      const { body, response } = await test.step("Send SignIn with malformed email", async () =>
        sendSignIn(request, {
          email: "not-an-email",
  ...
  test(
    ...
    { tag: Tags.regression },
    async ({ request }) => {
      const response = await test.step("Send SignIn without email variable", async () =>
        request.post(apiUrl, {
          data: {
  ...
  test(
    ...
    async ({ request }) => {
      ...
      const response = await test.step("Send SignIn without password variable", async () =>
        request.post(apiUrl, {
          data: {
            operationName: "SignIn",
            query: SIGN_IN_MUTATION,
  ...
  test(
    ...
    async ({ request }) => {
      ...
      const longEmail = "a".repeat(200) + "@" + "b".repeat(50) + ".example.com";

      const { body, response } = await test.step("Send SignIn with oversized email", async () =>
        sendSignIn(request, {
          email: longEmail,
          password: "AnyPassword1!",
          continueFrom: "",
  ...
  test(
    ...
    async ({ request }) => {
      ...
      await test.step("Verify the sign-in was rejected cleanly", async () => {
        ...
        const rejected = body === null || isRejected(body) || response.status() >= 400;
