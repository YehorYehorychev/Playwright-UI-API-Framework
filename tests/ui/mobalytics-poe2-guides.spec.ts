 ...
 */
test.describe("POE2 Navigation Tests", { tag: [Tags.ui, Tags.regression, Tags.poe2] }, () => {
  // ── Smoke ─────────────────────────────────────────────────────────────
  ...
  test("should display POE2 in-page sub-navigation links", {}, async ({ poe2Page }) => {
    await test.step("Navigate directly to POE2 page", async () => {
      await poe2Page.goto(`${config.baseURL}${TestData.urls.poe2}`);
      await poe2Page.waitForPageLoad();
    });
    ...
    await test.step("Verify sub-nav links are visible", async () => {
      ...
      for (const label of TestData.ui.poe2.subNavLinks) {
        const link = poe2Page.page.getByRole("link", { name: label, exact: true }).first();
  ...
  test(
    ...
    async ({ authenticatedPoe2Page: poe2Page }) => {
      ...
      await test.step("Verify guides page loaded", async () => {
        ...
        await expect(poe2Page.page).toHaveURL(TestData.urlPatterns.poe2Guides);
      });
    },
  );
  ...
  test(
    ...
    async ({ authenticatedPoe2Page: poe2Page }) => {
      ...
      const guideUrlPattern = /.*0-4-the-last-of-the-druids-content-livestream-summary$/;
